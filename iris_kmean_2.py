...
#sns.FacetGrid(iris,hue='pred').map(plt.scatter,'sepal_length','sepal_width').add_legend()
#plt.show()

#calculando a métrica de performance
metrica = sm.accuracy_score(iris.species, cluster_model.labels_)

metrica_ajust = metrics.adjusted_rand_score(iris.species, cluster_model.labels_)
print(metrica_ajust) # índice de rand ajustado é uma função que mede a similaridade das duas atribuições

#criando a confusion matrix

conf_mat = sm.confusion_matrix(iris.species, cluster_model.labels_)
print(conf_mat)
